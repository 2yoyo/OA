...
<div class="pd-20">
	 ...
	 <a href="javascript:;" onclick="admin_add('添加用户','application/emp/empAdd.jsp','800','500')" class="btn btn-primary radius">
	...
	<table class="table table-border table-bordered table-bg">
		...
		<c:forEach items="${page.list}" var="emp">
			<tr class="text-c">
				...
				<td>${emp.sex == 1?'男':'女'}</td>
				<td>
				<fm:formatDate value="${emp.hiredate}" pattern="yyyy-MM-dd"/>
				</td>
				<td>${emp.dname}</td>
				<td class="td-manage">
				<a style="text-decoration:none" onClick="admin_stop(this,'10001')" href="javascript:;" title="停用"><i class="Hui-iconfont">&#xe631;</i></a> 
				<a title="编辑" href="javascript:;" onclick="admin_edit('用户编辑','empController/getById/${emp.id}','1','800','500')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a> 
				<a title="删除" href="javascript:;" onclick="admin_del(this,${emp.id})" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
			</tr>
		</c:forEach>
		</tbody>
	</table>
...
function admin_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		debugger;
		$.get("empController/delete/"+id,"",function(data){
			showMsg(data);
		},"JSON")
	});
}
