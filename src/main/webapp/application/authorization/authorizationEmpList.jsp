	...
	<table class="table table-border table-bordered table-bg">
		...
		<c:forEach items="${page.list}" var="emp">
			<tr class="text-c">
				...
				<td>${emp.username}</td>
				<td>${emp.phone}</td>
				<td><a title="删除" href="javascript:;" onclick="admin_del(this,${emp.id})" class="ml-5" style="text-decoration:none">
...
function admin_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		...
		$.get("authorizationController/deleteAuthorization/"+id+"/"+roleId,"",function(data){
			showMsg(data);
 		},"JSON");
	});
}
